...
class ChatSession:
    ...
    def ask(self, question: str, on_token=None, on_standalone=None) -> dict:
        ...
        standalone = condense(self.history, question)
        if on_standalone:
        ...
        ans_parts = [standalone, [c["chunk_id"] for c in chunks], settings.gen_model]
        ...
        text = self._generate(messages, on_token)
        ...
        if chunks and not is_grounded(text, len(chunks)):
            ...
            retry = self._generate(strict, None)
        ...
        text = verify_answer(standalone, text, chunks)
    ...
    def _generate(self, messages: list[dict], on_token) -> str:
        parts: list[str] = []
        try:
            for chunk in chat(messages, temperature=0.0, max_tokens=600, stream=True):
                ...
                if delta:
                    ...
                    if on_token:
                        on_token(delta)
        except Exception:  # provider/stream hiccups shouldn't crash the chat
            pass
        ...
        try:
            text = message_text(chat(messages, temperature=0.0, max_tokens=600)).strip()
        except Exception:
            return ""
