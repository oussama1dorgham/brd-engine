...
class Settings:
    ...
    # Email transport selection. 'resend' sends over Resend's HTTPS API
    # (RESEND_API_KEY) — needed on hosts (Render/most PaaS free tiers) that block
    # outbound SMTP ports. 'smtp' uses the SMTP_* settings below (ON HOLD: blocked
    # on Render, usable on a VM — e.g. a future Gmail SMTP setup). With no usable
    # transport nor SMTP_HOST, send falls back to dev-log mode.
